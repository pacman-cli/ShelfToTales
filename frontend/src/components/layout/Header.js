...
import { wishlistService, orderService } from './../../api/api';
...
function Header(){
	...
	const fetchCounts = async () => {
		try {
			...
			if (cartRes && cartRes.data && cartRes.data.items) {
				...
				<div className="main-bar clearfix">
					<div className="container clearfix">
						...
						<div className="extra-nav">
							<div className="extra-cell">
								{user ? (
									<Link to={"/reading-room"} className="btn btn-primary btnhover">Start Reading</Link>
								) : (
									<Link to={"/shop-registration"} className="btn btn-primary btnhover">Join Community</Link>
								)}
							</div>
						</div>
